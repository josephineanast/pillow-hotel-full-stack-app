...
const BookingDatePicker = ({ room }: Props) => {
  ...
  const { data: { bookedDates: dates } = {} } = useGetBookedDatesQuery(id);
  ...
  return (
    <div className="booking-card shadow p-4">
      ...
      <hr />
      <p className="mt5 mb-3">Pick Check In & Check Out Date to Book:</p>
      ...
      {isAvailable === false && (
        <div className="alert alert-danger my-3">Room is unavailable.</div>
      )}

      {isAvailable && !isAuthenticated && (
        <div className="alert alert-danger my-3">Login to book this room.</div>
      )}
